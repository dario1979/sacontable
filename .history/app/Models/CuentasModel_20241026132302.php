...
class CuentasModel extends Model
{
    ...
    public function obtenerCuentasPadres()
    {
        ...
        $cuentasPadres = CuentasModel::where('recibe_saldo', 0)->get(['idcuenta', 'nro_cuenta', 'nombre']);
    ...
    public function obtenerProximoNroCuenta($padreId, $tipo)
    {
        ...
        // Obtener el prefijo del número de cuenta para las cuentas hijas
        $prefijoNroCuenta = $cuentaPadre->nro_cuenta;

        // Inicializar el siguiente número de cuenta hijo
        $siguienteNumero = 1;

        // Bucle para encontrar un número de cuenta disponible
        do {
            $nro_cuenta_siguiente = $prefijoNroCuenta . $siguienteNumero;

            // Verificar si el número de cuenta ya existe y tiene el mismo tipo
            $existe = CuentasModel::where('nro_cuenta', $nro_cuenta_siguiente)
                ->where('tipo', $tipo)
                ->exists();

            // Si existe, incrementar el número para el próximo intento
            if ($existe) {
                $siguienteNumero++;
            }
        } while ($existe);
