...
class CuentasModel extends Model
{
    ...
    public function obtenerProximoNroCuenta($padreId, $tipo)
    {
        ...
        // Buscar el último número de cuenta dentro del mismo nivel jerárquico y tipo
        $ultimoHijo = CuentasModel::where('nro_cuenta', '>=', $prefijoNroCuenta * 10)
            ->where('nro_cuenta', '<', ($prefijoNroCuenta + 1) * 10)
            ->where('tipo', $tipo)
            ->orderBy('nro_cuenta', 'desc')
            ->first();

        // Determinar el siguiente número de cuenta
        $nro_cuenta_siguiente = $ultimoHijo ? $ultimoHijo->nro_cuenta + 1 : $prefijoNroCuenta * 10;
