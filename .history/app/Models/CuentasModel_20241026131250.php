...
class CuentasModel extends Model
{
    ...
    public function obtenerProximoNroCuenta($padreId)
    {
        ...
        // Obtener el último `nro_cuenta` hijo de la cuenta padre
        $ultimoHijo = CuentasModel::where('nro_cuenta', 'LIKE', $cuentaPadre->nro_cuenta . '%')
            ->orderBy('nro_cuenta', 'desc')
            ->first();

        // Calcular el siguiente número de cuenta
        $nro_cuenta_siguiente = $ultimoHijo
            ? (int) $ultimoHijo->nro_cuenta + 1
            : $cuentaPadre->nro_cuenta . '1';
