...
class CuentasController extends Controller
{
    ...
    public function getCuentas(Request $request)
    {
        ...
        $result = array_map(function ($cuenta) {
            return [
                'label' => $cuenta['nro_cuenta'] . ' - ' . $cuenta['nombre'] .' - '. $cuenta['tipo'],  // Texto que se muestra en el autocomplete
                'value' => $cuenta['nro_cuenta'] . ' - ' .  $cuenta['nombre'] .' - '. $cuenta['tipo'],  // Valor que se completa en el campo
    ...
    {
        $cuentasM = new CuentasModel();
        $cuentasPadres = $cuentasM->obtenerCuentasPadres();

        return response()->json($cuentasPadres);
    }

    public function obtenerProximoNroCuenta(Request $request)
    {
    ...
    public function totalCuentas(Request $request){
        $cuentasM = new CuentasModel();
        $totalRecords = $cuentasM->getTotalRecords();

        return response()->json(array("totalCuentas"=>$totalRecords));
    }
}
