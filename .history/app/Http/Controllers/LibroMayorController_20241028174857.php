        ...
        if ($request->ajax()) {
            ...
            foreach ($cuentas as $cuenta) {
                foreach ($cuenta->movimientos as $movimiento) {
                    $movimiento->asiento->fecha = \Carbon\Carbon::createFromFormat('Y-m-d', $movimiento->asiento->fecha)->format('d/m/Y');
                }
            }
            return DataTables::of($cuentas)
                ->addColumn('movimientos', function ($cuenta) {
                    return view('reportes.libro_mayor', compact('cuenta'))->render();
                })
